...
CREATE UNIQUE INDEX IF NOT EXISTS courses_code_unique_idx ON public.courses (code) WHERE code IS NOT NULL AND code != '';

-- Ensure access_code is unique
ALTER TABLE public.courses DROP CONSTRAINT IF EXISTS courses_access_code_key;
CREATE UNIQUE INDEX IF NOT EXISTS courses_access_code_unique_idx ON public.courses (access_code);